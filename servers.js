...
// ====================================================================
// --- 🚀 ИСПРАВЛЕНИЕ: API МАРШРУТЫ ОБЪЯВЛЕНЫ ПЕРЕД СТАТИКОЙ ---
// ====================================================================
...
app.post('/api/user/get-or-create', async (req, res) => {
    ...
    try {
        let userResult = await pool.query("SELECT id, telegram_id, username, balance_uah as balance FROM users WHERE telegram_id = $1", [telegram_id]);
        if (userResult.rows.length > 0) {
            res.json(userResult.rows[0]);
        } else {
            const initialBalance = 1000.00;
            const newUserResult = await pool.query(
                "INSERT INTO users (telegram_id, username, balance_uah) VALUES ($1, $2, $3) RETURNING id, telegram_id, username, balance_uah as balance",
...
app.post('/api/user/inventory/sell', async (req, res) => {
    ...
    try {
        ...
        await client.query("UPDATE users SET balance_uah = balance_uah + $1 WHERE id = $2", [itemValue, user_id]);

        await client.query('COMMIT');

        const userResult = await pool.query("SELECT balance_uah as balance FROM users WHERE id = $1", [user_id]);
...
app.post('/api/case/open', async (req, res) => {
    ...
    try {
        ...
        const userResult = await client.query("SELECT id, balance_uah as balance FROM users WHERE id = $1 FOR UPDATE", [user_id]);
...
app.post('/api/contest/buy-ticket', async (req, res) => {
    ...
    try {
        ...
        const userResult = await client.query("SELECT id, balance_uah as balance FROM users WHERE telegram_id = $1 FOR UPDATE", [telegram_id]);
        if (userResult.rows.length === 0) throw new Error('Пользователь не найден');
        const user = userResult.rows[0];
...
adminRouter.use(checkAdminSecret); // Защищаем все маршруты в этом роутере

adminRouter.get('/users', async (req, res) => {
    try {
        const { rows } = await pool.query("SELECT id, telegram_id, username, balance_uah as balance FROM users ORDER BY id DESC");
        res.json(rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
adminRouter.post('/user/:userId/balance', async (req, res) => {
...
adminRouter.get('/case/items', async (req, res) => {
    const caseId = 1;
    try {
...
adminRouter.post('/case/items', async (req, res) => {
...
adminRouter.post('/game_settings', async (req, res) => {
...
adminRouter.post('/contest/create', async (req, res) => {
...
adminRouter.post('/contest/draw/:id', async (req, res) => {
...
// Регистрируем роутер для админских маршрутов
app.use('/api/admin', adminRouter);

// --- ОТДАЧА СТАТИЧЕСКИХ ФАЙЛОВ И HTML (ПОСЛЕ API) ---
